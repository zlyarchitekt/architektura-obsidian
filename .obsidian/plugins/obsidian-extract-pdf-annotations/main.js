...
var ANNOTS_TREATED_AS_HIGHLIGHTS = [
  "Highlight",
  "Underline",
  "Squiggly"
];
var PDFAnnotationPluginSetting = class {
  constructor() {
    ...
    this.noteTemplateExternalPDFs = "{{body}}\n\n* *noted by {{author}} at page {{pageNumber}} on {{filepath}}*\n\n";
    this.noteTemplateInternalPDFs = "{{body}}\n\n* *noted by {{author}} at page {{pageNumber}} on [[{{filepath}}]]*\n\n";
    this.highlightTemplateExternalPDFs = "> {{highlightedText}}\n\n{{body}}\n\n* *highlighted by {{author}} at page {{pageNumber}} on {{filepath}}*\n\n";
    this.highlightTemplateInternalPDFs = "> {{highlightedText}}\n\n{{body}}\n\n* *highlighted by {{author}} at page {{pageNumber}} on [[{{filepath}}]]*\n\n";
  ...
  parseCommaSeparatedStringToArray(desiredAnnotations) {
...
var PDFAnnotationPluginSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  addValueChangeCallback(component, settingsKey, cb) {
    component.onChange((value) => __async(this, null, function* () {
      this.plugin.settings[settingsKey] = value;
      this.plugin.saveSettings().then(() => {
        if (settingsKey === "desiredAnnotations") {
          this.plugin.settings.parsedSettings.desiredAnnotations = this.plugin.settings.parseCommaSeparatedStringToArray(
            value
          );
        }
        if (cb) {
  ...
  buildValueInput(component, settingsKey, cb) {
  ...
  display() {
    ...
    desiredAnnotationsInstructionsEl.append(
      createSpan({
        text: "You can specify which types of annotations should be extracted by the plugin. List the types exactly as listed here, separated by commas. The plugin supports the following types of annotations: "
      })
    );
    const desiredAnnotationsVariableUl = containerEl.createEl("ul");
    Object.entries(SUPPORTED_ANNOTS).forEach((variableData) => {
      const [key, description] = variableData, desiredAnnotationsVariableItem = desiredAnnotationsVariableUl.createEl("li");
    ...
    new import_obsidian.Setting(containerEl).setName("The following types of annotations should be extracted:").addTextArea((input) => {
    ...
    templateInstructionsEl.append(
      createSpan({
        text: "The following settings determine how the highlights and notes created by the plugin will be rendered. There are four types that you can specify, because you might want to have other templates for highlights and notes which include links to external files. Templates are interpreted using "
      })
    );
    templateInstructionsEl.append(
      createEl("a", {
    ...
    Object.entries(TEMPLATE_VARIABLES).forEach((variableData) => {
      const [key, description] = variableData, templateVariableItem = templateVariableUl.createEl("li");
    ...
    new import_obsidian.Setting(containerEl).setName("Template for notes of PDFs outside Obsidian:").addTextArea((input) => {
      input.inputEl.style.width = "100%";
      input.inputEl.style.height = "10em";
      this.buildValueInput(input, "noteTemplateExternalPDFs");
    });
    new import_obsidian.Setting(containerEl).setName("Template for notes of PDFs inside Obsidian:").addTextArea((input) => {
      input.inputEl.style.width = "100%";
      input.inputEl.style.height = "10em";
      this.buildValueInput(input, "noteTemplateInternalPDFs");
    });
    new import_obsidian.Setting(containerEl).setName("Template for highlights of PDFs outside Obsidian:").addTextArea((input) => {
      input.inputEl.style.width = "100%";
      input.inputEl.style.height = "10em";
      this.buildValueInput(input, "highlightTemplateExternalPDFs");
    });
    new import_obsidian.Setting(containerEl).setName("Template for highlights of PDFs inside Obsidian:").addTextArea((input) => {
      input.inputEl.style.width = "100%";
      input.inputEl.style.height = "10em";
      this.buildValueInput(input, "highlightTemplateInternalPDFs");
    });
    containerEl.createEl("h4", { text: "Structure settings" });
    new import_obsidian.Setting(containerEl).setName("Use structuring headlines").setDesc(
      "If disabled, no structuring headlines will be shown. Just the annotations in the specified template style."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useStructuringHeadlines).onChange((value) => {
        this.plugin.settings.useStructuringHeadlines = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Use Folder Name").setDesc(
      "If enabled, uses the PDF's folder name (instead of the PDF-Filename) for sorting"
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.useFolderNames).onChange((value) => {
        this.plugin.settings.useFolderNames = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Sort by Topic").setDesc(
      "If enabled, uses the notes first line as Topic for primary sorting"
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.sortByTopic).onChange((value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Notes export path").setDesc(
    ...
    new import_obsidian.Setting(containerEl).setName("Notes export name").setDesc(
    ...
    new import_obsidian.Setting(containerEl).setName("One note per annotation").setDesc(
      "If enabled, every annotation is exported to a separate note."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.oneNotePerAnnotation).onChange((value) => {
        this.plugin.settings.oneNotePerAnnotation = value;
        oneNotePerAnnotationExportName.settingEl.style.display = value ? "flex" : "none";
        this.plugin.saveData(this.plugin.settings);
      })
    );
    const oneNotePerAnnotationExportName = new import_obsidian.Setting(containerEl).setName("One note per annotation - export name").setDesc(
    ...
    new import_obsidian.Setting(containerEl).setName("Overwrite existing note").setDesc(
      "If enabled, the plugin will overwrite the content of an existing note with the same name."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.overwriteExistingNote).onChange((value) => {
        this.plugin.settings.overwriteExistingNote = value;
        this.plugin.saveData(this.plugin.settings);
      })
    );
    new import_obsidian.Setting(containerEl).setName("Extract tags in annotations as Obsidian tags").setDesc(
      "If enabled, the plugin will extract tags from the annotations and add them as Obsidian tags to the note's header."
    ).addToggle(
      (toggle) => toggle.setValue(this.plugin.settings.extractTagsFromAnnotationsAsObsidianTags).onChange((value) => {
...
// src/extractHighlight.ts
function searchQuad(minx, maxx, miny, maxy, items) {
  const mycontent = items.reduce(function(txt, x) {
    if (x.width == 0) return txt;
    if (!(miny <= x.transform[5] && x.transform[5] <= maxy)) return txt;
    if (x.transform[4] + x.width < minx) return txt;
    if (x.transform[4] > maxx) return txt;
    const start = x.transform[4] >= minx ? 0 : Math.round(
    ...
    if (x.transform[4] + x.width <= maxx) {
      ...
      const lenc = roundBasedOnLetterWidths(x, start, exactLength) - start;
...
function extractHighlight(annot, items) {
  ...
  const highlight = legacyQuadPoints.reduce((txt, quad) => {
...
// src/types.ts
var PDFFile = class _PDFFile {
  constructor(name, binaryContent, extension, path) {
    this.name = name;
    this.content = binaryContent;
    this.extension = extension;
    this.path = path;
  }
  static convertTFileToPDFFile(tFile, binaryContent) {
    let pdfFile = new _PDFFile(tFile.name, binaryContent, tFile.extension, tFile.path);
    return pdfFile;
  }
};
...
var PDFAnnotationPluginFormatter = class {
...
`;
  ...
  get noteFromExternalPDFsTemplate() {
    return (0, import_handlebars.compile)(
      this.settings.noteTemplateExternalPDFs,
      this.templateSettings
    );
  }
  get noteFromInternalPDFsTemplate() {
    return (0, import_handlebars.compile)(
      this.settings.noteTemplateInternalPDFs,
      this.templateSettings
    );
  }
  get highlightFromExternalPDFsTemplate() {
    return (0, import_handlebars.compile)(
      this.settings.highlightTemplateExternalPDFs,
      this.templateSettings
    );
  }
  get highlightFromInternalPDFsTemplate() {
    return (0, import_handlebars.compile)(
      this.settings.highlightTemplateInternalPDFs,
      this.templateSettings
    );
  }
  getTemplateVariablesForAnnotation(annotation) {
    const shortcuts = {
    ...
    return __spreadValues({ annotation }, shortcuts);
  }
  getContentForNoteFromExternalPDF(annotation) {
    return this.noteFromExternalPDFsTemplate(
      this.getTemplateVariablesForAnnotation(annotation)
    );
  }
  getContentForNoteFromInternalPDF(annotation) {
    return this.noteFromInternalPDFsTemplate(
      this.getTemplateVariablesForAnnotation(annotation)
    );
  }
  getContentForHighlightFromExternalPDF(annotation) {
    return this.highlightFromExternalPDFsTemplate(
      this.getTemplateVariablesForAnnotation(annotation)
    );
  }
  getContentForHighlightFromInternalPDF(annotation) {
...
// src/main.ts
var PDFAnnotationPlugin = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    // Template compilation options
    this.templateSettings = {
  ...
  loadSinglePDFFile(pdfFile) {
    return __async(this, null, function* () {
      ...
      const content = yield this.app.vault.readBinary(pdfFile);
      yield loadPDFFile(
      ...
      if (this.settings.oneNotePerAnnotation) {
        grandtotal.forEach((anno, index) => {
          ...
          const fileNameOfExportNote = this.getResolvedOneNotePerAnnotationExportName(pdfFile, index + 1) + ".md";
        ...
        yield this.saveHighlightsToFileAndOpenIt(
  ...
  loadAnnotationsFromSinglePDFFileFromClipboardPath(filePathFromClipboard) {
    return __async(this, null, function* () {
      const grandtotal = [];
      try {
    ...
    return __async(this, null, function* () {
      ...
      this.addCommand({
        ...
        editorCallback: (editor, view) => __async(this, null, function* () {
          ...
          const grandtotal = yield this.loadAnnotationsFromSinglePDFFileFromClipboardPath(
            clipText
          );
          this.sort(grandtotal);
          ...
          import_obsidian2.Vault.recurseChildren(folder, (file2) => __async(this, null, function* () {
            if (file2 instanceof import_obsidian2.TFile) {
              if (file2.extension === "pdf") {
                promises.push(
                  this.app.vault.readBinary(file2).then(
                    (content) => loadPDFFile(
      ...
      if (loadedSettings) {
        ...
        toLoad.forEach((setting) => {
          if (setting in loadedSettings) {
            this.settings[setting] = loadedSettings[setting];
          }
        });
        this.settings.parsedSettings = {
          desiredAnnotations: this.settings.parseCommaSeparatedStringToArray(
  ...
  get exportNameTemplate() {
    return (0, import_handlebars2.compile)(this.settings.exportName, this.templateSettings);
  }
  get oneNotePerAnnotationExportNameTemplate() {
    return (0, import_handlebars2.compile)(this.settings.oneNotePerAnnotationExportName, this.templateSettings);
  }
  getTemplateVariablesForExportName(file) {
    const shortcuts = {
      filename: file.basename
    };
    return __spreadValues({ file }, shortcuts);
  }
  getTemplateVariablesForOneNotePerAnnotationExportName(file, counter) {
    const shortcuts = {
      filename: file.basename,
      counter
    };
  ...
  getResolvedExportName(file) {
    return this.exportNameTemplate(
      this.getTemplateVariablesForExportName(file)
    );
  }
  getResolvedOneNotePerAnnotationExportName(file, counter) {
  ...
  getResolvedExportPath(pdfFile, fileNameOfExportNote) {
  ...
  saveHighlightsToFileAndOpenIt(filePath, mdString, overwriteExistingNote) {
    return __async(this, null, function* () {
      const fileExists = yield this.app.vault.adapter.exists(filePath);
      if (fileExists) {
        if (overwriteExistingNote) {
          yield this.app.vault.adapter.write(filePath, mdString);
        } else {
  ...
  appendHighlightsToFile(filePath, note) {
    return __async(this, null, function* () {
